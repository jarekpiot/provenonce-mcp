...
export const PROVENONCE_TOOLS = [
...

  {
    ...
    description:
      ...
      'and a permanent identity record. If payment_tx is omitted, returns payment instructions (fee amount + address). ' +
...

  {
    ...
    description:
      'Compute VDF beats and submit to the Registry to credit this agent\'s lifetime beat count. ' +
      'Unlike provenonce_beats_proof (which gets a receipt for spawn/resync), this extends the agent\'s ' +
      'persistent beat chain and increases the "Lifetime Beats" metric visible in the Registry.',
    inputSchema: {
      type: 'object',
      properties: {
        count: {
  ...
  },
  {
    ...
    description:
      'Submit heartbeats for multiple child agents in a single call. ' +
      'One payment covers all children. Requires active sponsorships for each child. ' +
      'Max 20 children per batch.',
    inputSchema: {
      type: 'object',
      properties: {
        children: {
